...
public class Query2{
	...
	void query2(String produit, String dateD, String dateF) throws ParseException {
			...
			for(StreamEntry streamUneLigneI : streamI.get(0).getValue()){
				...
				for(StreamEntry streamUneLigneF : streamF.get(0).getValue()){
	                	if(!streamUneLigneF.getFields().get("asin").equals(produit))
	        				continue;
	                    if(streamUneLigneI.getFields().get("Orderline.asin").equals(produit) && streamUneLigneF.getFields().get("asin").equals(produit) && streamUneLigneI.getFields().get("PersonId").equals(streamUneLigneF.getFields().get("PersonId"))) {
	                    	Date dateT = sdf.parse(streamUneLigneI.getFields().get("OrderDate"));
	    					
	    					if(!(dateT.compareTo(date2) > 0 || dateT.compareTo(date) < 0))
	    						System.out.println(streamUneLigneI.getFields().get("PersonId") + " a acheté le produit " + produit);
	                    }
	                }
	            }
